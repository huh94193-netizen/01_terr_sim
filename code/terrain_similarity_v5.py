...
def preprocess_srtm(srtm_path_pattern, target_extent, buffer_deg, wrf_res, target_crs):
    ...
    logger.info(f"Target Extent (Buffered): {target_bounds_buf}")

    logger.info("Listing SRTM files...")
    ...
    for fp in srtm_files:
        try:
            ...
            logger.warning(f"Metadata read failed for {fp}, skipping")
            continue

    if not candidate_files:
        raise FileNotFoundError("No intersecting SRTM tiles found!")
    logger.info(f"Selected {len(candidate_files)} tiles. Loading...")
    ...
    logger.info(f"Reprojecting to {target_crs} and resampling...")
    ...
    logger.info(f"Preprocessing done. Shape: {elev.shape}")
...
def generate_sliding_windows(elev, x_coords, y_coords, window_size_m, step_size_m, wrf_res):
    ...
    logger.info(f"Generated {len(window_df)} windows.")
...
if __name__ == "__main__":
    try:
        logger.info("=" * 60)
        logger.info("Starting Terrain Similarity Analysis")
        logger.info("=" * 60)
        ...
        logger.info(f"Feature extraction done. Valid samples: {len(valid_df)}")
        ...
        if best_pair is not None:
            ...
            result_df = pd.DataFrame({
                ...
                "Similarity_Score": [best_score, best_score]
            })
            csv_path = os.path.join(RESULT_ROOT, "terrain_similarity_best_pair.csv")
            result_df.to_csv(csv_path, index=False)

            # 2. 局部对比图 (English Labels)
            logger.info("Generating detail comparison plot...")
            ...
            axes[0, 0].set_title(f"Area 1 Elevation (Score: {best_score:.3f})")
            plt.colorbar(im1, ax=axes[0, 0], label="Elevation (m)")
            
            # Area 1 Slope
            im2 = axes[0, 1].imshow(slope1, cmap="viridis")
            axes[0, 1].set_title("Area 1 Slope")
            plt.colorbar(im2, ax=axes[0, 1], label="Slope (deg)")
            
            # Area 2 Elevation
            im3 = axes[1, 0].imshow(elev2, cmap="terrain", vmin=vmin, vmax=vmax)
            axes[1, 0].set_title("Area 2 Elevation")
            plt.colorbar(im3, ax=axes[1, 0], label="Elevation (m)")
            
            # Area 2 Slope
            im4 = axes[1, 1].imshow(slope2, cmap="viridis")
            axes[1, 1].set_title("Area 2 Slope")
            plt.colorbar(im4, ax=axes[1, 1], label="Slope (deg)")
            
            plt.tight_layout()
            plt.savefig(os.path.join(RESULT_ROOT, "detail_comparison.png"), dpi=300)

            # 3. 全局位置概览图 (English Labels)
            logger.info("Generating overview location map...")
            ...
            # 颜色条改为英文标签
            plt.colorbar(im_ov, ax=ax_ov, label="Elevation (m)", fraction=0.046, pad=0.04)
            ...
            # 标题和坐标轴改为英文
            ax_ov.set_title("Location of Similar Terrain Areas")
            ...
            logger.info(f"Overview map saved to: {overview_path}")
            
        else:
            logger.info("No non-overlapping similar areas found!")
        
        logger.info("=" * 60)
        logger.info("Analysis Completed")

    except Exception as e:
        logger.error(f"\nExecution failed: {str(e)}", exc_info=True)
        raise